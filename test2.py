...
class Win(Screen, GridLayout):
    ...
    def score(self, value):
        ...
        scoreAchieved = math.log(1+(10000/(Global.end-Global.start)))
        ...
        if newHighScore == True:
            ...
            self.no_button = Button(text="Nah...Exit game", on_press=self.exit_game)
            self.layout.add_widget(self.score_label)
            self.layout.add_widget(self.hi_label)
            self.layout.add_widget(self.yes_button)
            self.layout.add_widget(self.no_button)
        else:
            diff = highest[-1] - scoreAchieved
            string = "You are " + str(diff) + " behined the highest score in this difficulty, try harder next time!"
            self.nexttime_label = Label(text=string)
            self.exit_button = Button(text="exit game", on_press=self.exit_game)

            self.layout.add_widget(self.score_label)
            self.layout.add_widget(self.nexttime_label)
